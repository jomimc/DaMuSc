...
def parse_args():
    ...
    parser.add_argument("--use_all_modes", action="store", default=False,
                        help="Include all possible modes? Must be followed by True or False")
    parser.add_argument("--use_specific_modes", action="store", default=True,
                        help="Include modes specified in sources? Must be followed by True or False")
...
def process_theory_scales(use_all_modes=False, use_compound=False):
    df = pd.read_csv(PATH_DATA.joinpath('theory_scales.csv'))
    cols_to_keep = ['TheoryID', 'Name', 'Region', 'Country', 'SocID', 'Tuning']
    new_cols = ['ScaleID'] + ['n_notes', 'scale', 'adjacent_intervals', 'tonic_intervals', 'all_intervals'] + cols_to_keep
    ...
    for i, row in df.iterrows():
        ...
        # WORK IN PROGRESS!!!
        if not row['Reduced_scale'] and not use_compound:
            continue
        for ints in [row['Ascending Intervals'], row['Descending Intervals']]:
            ...
            if isinstance(ints, str):
                for scale in tuning_system.get_theory_scale_in_cents(ints, row.Tonic, row.Tuning):
                    if use_all_modes:
                        for mode in utils.get_all_modes(scale):
                            N, adj_ints, tonic_ints, all_ints = utils.process_scale(mode)
                            idx = len(df_new)
                            df_new.loc[idx] = [f"OT{idx:04d}", N, mode, adj_ints, tonic_ints, all_ints] + vals
                    else:
                        N, adj_ints, tonic_ints, all_ints = utils.process_scale(scale)
                        idx = len(df_new)
                        df_new.loc[idx] = [f"OT{idx:04d}", N, scale, adj_ints, tonic_ints, all_ints] + vals
    df_new['Theory'] = 'Y'
    return df_new


### Create new DataFrame with intervals in cents for measured scales
def process_measured_scales(oct_cut=50, use_specific_modes=True, use_all_modes=False):
    df = pd.read_csv(PATH_DATA.joinpath('measured_scales.csv'))
    df.Intervals = df.Intervals.apply(utils.str_to_ints)
    cols_to_keep = ['MeasuredID', 'TheoryID', 'Name', 'Region', 'Country', 'SocID', 'Measured_type']
    new_cols = ['ScaleID'] + ['n_notes', 'scale', 'adjacent_intervals', 'tonic_intervals', 'all_intervals'] + cols_to_keep
    df_new = pd.DataFrame(columns=new_cols)
    for row in df.itertuples():
        vals = list(df.loc[row[0], cols_to_keep].values)
        for scale in utils.extract_scale_from_measurement(row, oct_cut, use_specific_modes, use_all_modes):
            N, adj_ints, tonic_ints, all_ints = utils.process_scale(scale)
            idx = len(df_new)
            df_new.loc[idx] = [f"OM{idx:04d}", N, scale, adj_ints, tonic_ints, all_ints] + vals
    df_new['Theory'] = 'N'
    return df_new
...
def remove_duplicates(df):
    ...
    for row in df.itertuples():
        if row[0] in to_bin:
            continue
        idx = (df.SocID==row.SocID) & (df.adjacent_intervals.apply(lambda x: same_ints(x, row.adjacent_intervals)))
        if sum(idx)>1:
...
def process_data(oct_cut=50, n_min=4, n_max=9,
                 use_specific_modes=True, use_all_modes=False, use_compound=False):

    df = pd.concat([remove_duplicates(process_theory_scales(use_all_modes, use_compound)),
                    remove_duplicates(process_measured_scales(oct_cut, use_specific_modes, use_all_modes))],
                   ignore_index=True)
    ...
    df['min_int'] = df.adjacent_intervals.apply(min)
    df['max_int'] = df.adjacent_intervals.apply(max)
...
def reformat_df_and_save(df):
    cols = ['scale', 'adjacent_intervals', 'tonic_intervals', 'all_intervals']
    for c in cols:
...
if __name__ == '__main__':
    ...
    df = process_data(oct_cut=args.octave_cutoff, n_min=args.n_min, n_max=args.n_max,
                      use_specific_modes=args.use_specific_modes, use_all_modes=args.use_all_modes,
